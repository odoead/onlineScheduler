...
import {
  ...
  ɵɵtextInterpolate1,
  ɵɵviewQuery
} from "./chunk-QCG5SZF6.js";
import {
  require_cjs
} from "./chunk-IULEQ2BZ.js";
import {
  __objRest,
  __spreadProps,
  __spreadValues,
  __toESM
} from "./chunk-5VUDFOOO.js";
...
var NgxMaterialTimepickerService = class {
  constructor() {
    this.hourSubject = new import_rxjs.BehaviorSubject(DEFAULT_HOUR);
    this.minuteSubject = new import_rxjs.BehaviorSubject(DEFAULT_MINUTE);
    this.periodSubject = new import_rxjs.BehaviorSubject(TimePeriod.AM);
  }
  set hour(hour) {
...
var NgxMaterialTimepickerEventService = class {
  constructor() {
    this.backdropClickSubject = new import_rxjs.Subject();
    this.keydownEventSubject = new import_rxjs.Subject();
  }
  get backdropClick() {
    return this.backdropClickSubject.asObservable().pipe((0, import_operators.shareReplay)({
      bufferSize: 1,
      refCount: true
    }));
  }
  get keydownEvent() {
    return this.keydownEventSubject.asObservable().pipe((0, import_operators.shareReplay)({
      bufferSize: 1,
      refCount: true
    }));
  }
...
var NgxMaterialTimepickerDialControlComponent = class {
  ...
  ngOnInit() {
    if (this.isEditable) {
      ...
      this.timeControl.valueChanges.pipe((0, import_operators.tap)((value) => {
        if (value.length > 2) {
          this.updateInputValue(value.slice(-1));
        }
      }), (0, import_operators.debounceTime)(500), (0, import_operators.distinctUntilChanged)(), (0, import_operators.filter)((value) => !isTimeDisabledToChange(this.time, value, this.timeList)), (0, import_operators.tap)((value) => this.time = this.timeParserPipe.transform(value, this.timeUnit).toString())).subscribe(() => this.updateTime());
    }
  }
...
var NgxMaterialTimepickerContainerComponent = class {
  ...
  ngOnInit() {
    ...
    this.defineTime();
    this.selectedHour = this.timepickerService.selectedHour.pipe((0, import_operators.shareReplay)({
      bufferSize: 1,
      refCount: true
    }));
    this.selectedMinute = this.timepickerService.selectedMinute.pipe((0, import_operators.shareReplay)({
      bufferSize: 1,
      refCount: true
    }));
    this.selectedPeriod = this.timepickerService.selectedPeriod.pipe((0, import_operators.shareReplay)({
      bufferSize: 1,
      refCount: true
    }));
    this.timepickerBaseRef.timeUpdated.pipe((0, import_operators.takeUntil)(this.unsubscribe)).subscribe(this.setDefaultTime.bind(this));
  }
  onHourChange(hour) {
...
var NgxMaterialTimepickerComponent = class {
  constructor(eventService, domService) {
    ...
    this.timeUpdated = new import_rxjs.Subject();
    this.isEsc = true;
  ...
  subscribeToEvents() {
    (0, import_rxjs.merge)(this.eventService.backdropClick, this.eventService.keydownEvent.pipe((0, import_operators.filter)((e) => e.keyCode === ESCAPE && this.isEsc))).pipe((0, import_operators.takeUntil)(this.unsubscribe)).subscribe(() => this.close());
  }
...
var NgxTimepickerFieldComponent = class {
  ...
  ngOnInit() {
    ...
    this.hour$ = this.timepickerService.selectedHour.pipe((0, import_operators.tap)((clockTime) => this.selectedHour = clockTime.time), (0, import_operators.map)(this.changeDefaultTimeValue.bind(this)), (0, import_operators.tap)(() => this.isTimeRangeSet && this.updateAvailableMinutes()));
    this.minute$ = this.timepickerService.selectedMinute.pipe((0, import_operators.map)(this.changeDefaultTimeValue.bind(this)), (0, import_operators.tap)(() => this.isFirstTimeChange = false));
    if (this.format === 12) {
      this.timepickerService.selectedPeriod.pipe((0, import_operators.distinctUntilChanged)(), (0, import_operators.tap)((period) => this.period = period), (0, import_operators.tap)((period) => this.isChangePeriodDisabled = this.isPeriodDisabled(period)), (0, import_operators.takeUntil)(this.unsubscribe$)).subscribe(() => this.isTimeRangeSet && this.updateAvailableTime());
